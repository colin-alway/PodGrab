...
# Colin Alway : colin.alway@gmail.com
# March 2014  : replaced all TAB characters with 4 spaces in source code
# April 2014  : current_directory = os.getcwd(); -v/--verbose argument added
...
class Global(object):
    # cleaner than raw globals...
    verbose = False
    current_directory = ''  # Added 2011-10-06 Werner Avenant
    m3u_file = ''           # Added 2011-10-06 Werner Avenant
#class
...
def main(argv):
    ...
    Global.m3u_file = str(now)[:10] + '.m3u' 
    #current_directory = os.path.realpath(os.path.dirname(sys.argv[0]))
    Global.current_directory = os.getcwd()
    download_directory = Global.current_directory + os.sep + DOWNLOAD_DIRECTORY
    ...
    if not has_error:
        if mode == MODE_UNSUBSCRIBE:
            ...
            export_opml_file(cursor, connection, Global.current_directory)
...
def has_mail_users(cur, conn):
    ...
    # 2014-04-03 : added check for 'None'
    if cur.fetchone() == "0" or cur.fetchone() == None:
        return 0
    else:
        return 1
